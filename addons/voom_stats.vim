...
python << EOF
# NOTE: module voom_vim.py is imported in Vim as _VOoM
def voom_PrintStats():
    ...
    nodesWithChildren = len([i for i in xrange(1,len(bnodes)+1) if _VOoM.nodeHasChildren(VO,i)])
    vim.command("let l:nodesWithChildren=%s" %nodesWithChildren)
    nodesWithoutChildren = len([i for i in xrange(1,len(bnodes)+1) if not _VOoM.nodeHasChildren(VO,i)])
