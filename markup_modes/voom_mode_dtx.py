...
"""
VOoM markup mode for .dtx (Documented LaTeX) files. It is almost identical to
the LaTeX mode. The main difference is that only lines that begin with the
comment character % are considered for headlines.
See |voom-mode-dtx|,  ../../doc/voom.txt#*voom-mode-dtx*
"""
...
try:
    import vim
    if vim.eval('exists("g:voom_dtx_sections")')=='1':
        SECTIONS = vim.eval("g:voom_dtx_sections")
    if vim.eval('exists("g:voom_dtx_elements")')=='1':
        ELEMENTS = vim.eval("g:voom_dtx_elements")
    if vim.eval('exists("g:voom_dtx_verbatims")')=='1':
        VERBATIMS = vim.eval("g:voom_dtx_verbatims")
except ImportError:
    pass
import re
