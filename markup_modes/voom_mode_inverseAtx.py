...
def hook_doBodyAfterOop(VO, oop, levDelta, blnum1, tlnum1, blnum2, tlnum2, blnumCut, tlnumCut):
    ...
    ### --- the end ---
    if invalid_levs:
        vim.command("call Voom_ErrorMsg('VOoM (inverseAtx): Disallowed levels have been corrected after ''%s''')" %oop)
        invalid_levs = ', '.join(['%s' %i for i in invalid_levs])
        vim.command("call Voom_ErrorMsg('     level set to maximum (%s) for nodes: %s')" %(MAX, invalid_levs))
